...
export function createDatabase() {
  ...
  // Enable foreign keys and ensure proper synchronization
  db.pragma('foreign_keys = ON');
  db.pragma('journal_mode = WAL'); // Write-Ahead Logging for better concurrency
  db.pragma('synchronous = FULL'); // Ensure data is written to disk
  
  // Increase limits for storing large data (like base64 images)
  db.pragma('max_page_count = 2147483646'); // Maximum pages
  db.pragma('page_size = 4096'); // 4KB pages
  
  // Set a reasonable cache size (negative means KB, positive means pages)
