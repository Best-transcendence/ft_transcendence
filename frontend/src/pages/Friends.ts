...
import { initPresence, teardownPresence, isUserOnline } from "../services/presence";
import { sendWSMessage, getSocket } from "../services/ws";
...
function friendCard(friend: Friend, isOnline = false) {
  const statusClass = isOnline ? "text-green-400" : "text-red-400";
  return `
  <div data-user-id="${friend.id}" class="bg-slate-900 backdrop-blur-md rounded-2xl p-4 shadow-[0_0_30px_10px_#7037d3] h-[170px] relative overflow-hidden">
    <div class="flex items-start gap-4 absolute left-5 top-5">
      ...
      <div class="mr-3">
        <h3 class="text-white font-semibold">
          ${friend.name}
          <span id="friend-status-${friend.id}" class="inline-block ml-1 text-sm ${statusClass}" role="status" aria-label="${isOnline ? "Online" : "Offline"}">●</span>
        </h3>
        <p class="text-gray-400 text-sm break-words" style="word-break: break-word; overflow-wrap: break-word;"><i>${friend.bio}</i></p>
      </div>
    </div>
